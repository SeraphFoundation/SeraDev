...
VERSION_FILE="https://raw.githubusercontent.com/seraio/sera-network/master/Version"
VERSION_FILE_LEGACY="https://raw.githubusercontent.com/seraio/sera-network/master/Version_legacy"
DOCKER_IMAGE_ORACLE_VERSION="3.0.0"
DOCKER_IMAGE_sera_APP_VERSION="1.0.0"
DOCKER_IMAGE_sera_PARITY_VERSION="1.0.0"
DOCKER_IMAGE_NET_STATS_VERSION="1.0.0"

ENV_FILE=".env"
DOCKER_IMAGE_PARITY_REPO="seranet/node"
DOCKER_CONTAINER_PARITY="seranet"
DOCKER_IMAGE_APP_REPO="seranet/validator-app"
DOCKER_CONTAINER_APP="seraapp"
DOCKER_IMAGE_NETSTAT_REPO="seranet/netstat"
DOCKER_CONTAINER_NETSTAT="seranetstat"
DOCKER_COMPOSE_ORACLE="https://raw.githubusercontent.com/seraio/sera-bridge/master/native-to-erc20/oracle/docker-compose.keystore.yml"
DOCKER_IMAGE_ORACLE_REPO="seranet/native-to-erc20-oracle"
DOCKER_CONTAINER_ORACLE="seraoracle"
DOCKER_LOG_OPTS="--log-opt max-size=10m --log-opt max-file=25 --log-opt compress=true"
BASE_DIR=$(pwd)/seranet
...
BOOTNODE_FILE_sera="https://raw.githubusercontent.com/seraio/sera-network/master/config/bootnodes.txt"
BOOTNODE_FILE_SPARK="https://raw.githubusercontent.com/seraio/sera-network/master/config/spark/bootnodes.txt"
...
function getAndUpdateBlockNumbers {
  ...
  echo "Grabbing current sera block number"
  seraBLOCK=$((`curl -s --data '{"method":"eth_blockNumber","params":[],"id":1,"jsonrpc":"2.0"}' -H "Content-Type: application/json" -X POST $HOME_RPC_URL | { grep -o "\w*0x\w*" || true; }`))
  ...
  if [[ "$seraBLOCK" == 0 ]]; then
          ...
          displayErrorAndExit "Could not pull sera block please check your sera RPC config"
  fi

  echo "ETH BLOCK = $ETHBLOCK"
  echo "sera BLOCK = $seraBLOCK"
  ...
  sed -i "s/^HOME_START_BLOCK.*/HOME_START_BLOCK=${seraBLOCK}/" "$ENV_FILE"
...
function parseArguments {
  ...
  if ! [ -z "$TESTNET" ] ; then
    if [[ $TESTNET == true ]] ; then
      ...
      VERSION_FILE="https://raw.githubusercontent.com/seraio/sera-network/master/Version_testNet"
      DOCKER_IMAGE_PARITY_REPO="seranet/spark-node"
      DOCKER_IMAGE_NETSTAT_REPO="seranet/spark-netstat"
      DOCKER_IMAGE_APP_REPO="seranet/spark-validator-app"
    fi
  fi
...
function pullSnapShot {
  ...
  if [[ $ROLE != explorer ]] ; then
    ...
    if [[ -d "$DATABASE_DIR/seraNetwork/db" ]] ; then
      rm -r "$DATABASE_DIR/seraNetwork/db"
    fi
    mkdir -p "$DATABASE_DIR/seraNetwork"
    echo -e "\nDownloading snapshot"
    wget -O db.tar.gz "$SNAPSHOT_NODE"
    echo -e "\nExtracting"
    tar -xzvf db.tar.gz -C "$DATABASE_DIR/seraNetwork"
    echo -e "\nDeleting temp file"
    rm db.tar.gz
  else
...
function setup {
  ...
  echo "Parity version = $DOCKER_IMAGE_sera_PARITY_VERSION"
  echo "sera app version = $DOCKER_IMAGE_sera_APP_VERSION"
  echo "Netstats version = $DOCKER_IMAGE_NET_STATS_VERSION"

  # Pull the docker images.
  echo -e "\nPull the docker images..."
  DOCKER_IMAGE_PARITY="$DOCKER_IMAGE_PARITY_REPO:$DOCKER_IMAGE_sera_PARITY_VERSION"
  DOCKER_IMAGE_NETSTAT="$DOCKER_IMAGE_NETSTAT_REPO:$DOCKER_IMAGE_NET_STATS_VERSION"
  DOCKER_IMAGE_APP="$DOCKER_IMAGE_APP_REPO:$DOCKER_IMAGE_sera_APP_VERSION"
  ...
  if [[ $ROLE == validator ]] || [[ $ROLE == bridge-validator ]] ; then
    ...
    export VALIDATOR_KEYSTORE_DIR=$CONFIG_DIR/keys/seraNetwork
    count=$(cat $ENV_FILE | grep "VALIDATOR_KEYSTORE_DIR" | wc -l)
    if [ $count -lt 1 ]; then
      echo "VALIDATOR_KEYSTORE_DIR=$CONFIG_DIR/keys/seraNetwork" >> $ENV_FILE
    fi
  else
...
function run {
  ...
  # Pull bootnodes
  if [[ $TESTNET != true ]] ; then
    wget -O bootnodeFile $BOOTNODE_FILE_sera
  else
  ...
  if [[ $ROLE == bridge-validator ]] ; then
    $PERMISSION_PREFIX docker run \
      ...
      --role "$ROLE" \
      --parity-version "$DOCKER_IMAGE_sera_PARITY_VERSION" \
      --seraapp-version "$DOCKER_IMAGE_sera_APP_VERSION" \
